...
class RenterNotification(Namespace):
    def on_connect(self):
        emit("Notification has established")

    def on_notifying(self, renter_id):
        print(str(renter_id))
        orders = []
        # for order in OrderModel.get_not_notified_orders(renter_id):
        #     order.set_notified()
        #     orders.append(order.json())
        # emit("renter notification", {"orders": orders})


class LenderNotification:
    pass
